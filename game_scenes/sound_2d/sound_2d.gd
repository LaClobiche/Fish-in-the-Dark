...
@export var panning_strength: float = 4.0:
	set(value):
		value = clampf(value, 1.0, 10.0)
		panning_strength = value
		set_panning_strength(value)
@export var attenuation: float = 2.0:
	set(value):
	...
	for player in get_children():
		if player is AudioStreamPlayer2D:
			if not player.playing:
				...
				clear_tweens()
				if fade_in or fade_out_from != 0.0:
...
func set_attenuation(value: float):
	for player in get_children():
		if player is AudioStreamPlayer2D:
			player.attenuation = value


func set_pitch_scale(value: float):
	for player in get_children():
		if player is AudioStreamPlayer2D:
