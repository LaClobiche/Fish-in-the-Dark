...
var effects := {
	...
	"rod_damage" : load("res://sounds/effects/rod_damage.tres"),
	"rod_damage_crack" : load("res://sounds/effects/rod_damage_crack.tres"),
...
# contains array of voices
var voice_queue: Array[Array] = []
# contain current array of voices
var voice_array: Array = []

@onready var voice: AudioStreamPlayer2D = load("res://sounds/voice.tscn").instantiate()
@onready var music : AudioStreamPlayer2D = load("res://sounds/music.tscn").instantiate()
@onready var ambience : AudioStreamPlayer2D = load("res://sounds/ambience.tscn").instantiate()
@onready var sound_effects: Node = load("res://sounds/sound_effects.tscn").instantiate()
...
func play_voice(voice_audio, with_helper: bool = Saves.data["options"]["helper"]):
	if voice_audio is VoiceArray:
		voice_queue.append(voice_audio.get_voice_array())
	elif voice_audio is VoiceResource:
		if not with_helper and voice_audio.help:
			pass
		else:
			var voice_to_append: AudioStream = voice_audio.get_audio(Global.touchscreen)
			if voice_to_append != null:
	...
	if not voice.playing:
			voice.stream = get_next_voice()
			voice.playing = true


func play_next_voice():
	var audio: AudioStream = get_next_voice()
	if audio != null:
		voice.stream = audio
		voice.playing = true
	else:
...
func fade_out_tween(tween: Tween, node: Node):
	tween.tween_property(node, "volume_db", -80.0, 0.5).from_current()
